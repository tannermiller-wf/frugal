...
impl FContext {
    ...
    pub fn add_request_header<S1: Into<String>, S2: Into<String>>(
        &mut self,
        name: S1,
        value: S2,
    ) -> &mut Self {
    ...
    pub fn add_response_header<S1: Into<String>, S2: Into<String>>(
        &mut self,
        name: S1,
        value: S2,
    ) -> &mut Self {
...
mod test {
    ...
    fn test_request_header() {
        ...
        assert_eq!(Some("bar"), ctx.request_header("foo"));
        assert_eq!(Some("qux"), ctx.request_header("baz"));
        assert_eq!(Some(&"bar".to_string()), ctx.request_headers().get("foo"));
        assert_eq!(Some(cid), ctx.request_header(CID_HEADER));
    }
    ...
    fn test_response_header() {
        ...
        assert_eq!(Some("bar"), ctx.response_header("foo"));
        assert_eq!(Some("qux"), ctx.response_header("baz"));
    }
