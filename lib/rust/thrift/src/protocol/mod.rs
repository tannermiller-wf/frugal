...
impl<P, T> TOutputProtocol<T> for Box<P>
where
    T: TWriteTransport,
...
mod tests {
    ...
    fn takes_input_protocol<R, T>(t: &mut T)
    where
        R: TReadTransport,
        T: TInputProtocol<R>,
    {
    ...
    fn takes_output_protocol<W, T>(t: &mut T)
    where
        W: TWriteTransport,
        T: TOutputProtocol<W>,
    {
