...
{
    strict: bool,
    pub transport: R, // FIXME: shouldn't be public
}
...
impl<R> TInputProtocol<R> for TBinaryInputProtocol<R>
where
    R: TReadTransport,
{
...
impl TBinaryInputProtocolFactory {
    ...
    pub fn create<R>(&self, transport: R) -> TBinaryInputProtocol<R>
    where
        R: TReadTransport,
    {
        TBinaryInputProtocol::new(transport, true)
    }
}
...
impl<T> TOutputProtocol<T> for TBinaryOutputProtocol<T>
where
    T: TWriteTransport,
{
...
impl TBinaryOutputProtocolFactory {
    ...
    pub fn create<W>(&self, transport: W) -> TBinaryOutputProtocol<W>
    where
        W: TWriteTransport,
    {
        TBinaryOutputProtocol::new(transport, true)
    }
}
...
mod tests {
    ...
    fn must_write_strict_message_call_begin() {
        ...
        let expected: [u8; 16] = [
            0x80, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x04, 0x74, 0x65, 0x73, 0x74, 0x00, 0x00,
            0x00, 0x01,
        ];
    ...
    fn must_write_non_strict_message_call_begin() {
        ...
        let expected: [u8; 13] = [
            0x00, 0x00, 0x00, 0x04, 0x74, 0x65, 0x73, 0x74, 0x01, 0x00, 0x00, 0x00, 0x01,
        ];
    ...
    fn must_write_strict_message_reply_begin() {
        ...
        let expected: [u8; 16] = [
            0x80, 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0x04, 0x74, 0x65, 0x73, 0x74, 0x00, 0x00,
            0x00, 0x0A,
        ];
    ...
    fn must_write_non_strict_message_reply_begin() {
        ...
        let expected: [u8; 13] = [
            0x00, 0x00, 0x00, 0x04, 0x74, 0x65, 0x73, 0x74, 0x02, 0x00, 0x00, 0x00, 0x0A,
        ];
    ...
    fn must_round_trip_bytes() {
        ...
        let bytes: [u8; 25] = [
            0x20, 0xFD, 0x18, 0x84, 0x99, 0x12, 0xAB, 0xBB, 0x45, 0xDF, 0x34, 0xDC, 0x98, 0xA4,
            0x6D, 0xF3, 0x99, 0xB4, 0xB7, 0xD4, 0x9C, 0xA5, 0xB3, 0xC9, 0x88,
        ];
