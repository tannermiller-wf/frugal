...
use transport::{
    TIoChannel, TReadTransport, TReadTransportFactory, TTcpChannel, TWriteTransport,
    ...
    OPF: TOutputProtocolFactory + 'static,
{
    ...
    fn new_protocols_for_connection<R, W, I, O>(&mut self, stream: TcpStream) -> ::Result<(I, O)>
    where
        R: TReadTransport,
        W: TWriteTransport,
        I: TInputProtocol<R>,
        O: TOutputProtocol<W>,
    {
        ...
        let r_tran = self.r_trans_factory.create(r_chan);
        let i_prot = self.i_proto_factory.create(r_tran);

        // output protocol and transport
        let w_tran = self.w_trans_factory.create(w_chan);
...
fn handle_incoming_connection<PRC, R, W, I, O>(processor: Arc<PRC>, i_prot: I, o_prot: O)
where
    PRC: TProcessor,
    R: TReadTransport,
    W: TWriteTransport,
